(function() {
	...
	window.mobileSlide = function(ul, options) {
		if(ul.nodeType != 1 || ul.tagName.toLowerCase() != 'ul') {
			console.error('mobileSlide\'s param is not a HTML UL Element.');
			return;
		}
		if(ul.children.legnth === 0) {
			console.error('ul > li\'s number must > 0.');
		}
		//set style
	...
	function prevMove(ul, pLi, cLi, lx, callback) {
		pLi.style.transition = cLi.style.transition = pLi.style.webkitTransition = cLi.style.webkitTransition = '0.6s ease all';
